...
- **更新要把版本号写上。**这条命令是在 profile 里执行 `pnpm add`，而 pnpm 在记录的版本范围已被满足时不会动已安装的包——对已装的包跑 `add volens-dsh` 只会说 "Lockfile is up to date"，什么都不改。要更新就写版本号（`add volens-dsh@0.4.0`）；如果那个版本比 profile 的供应链策略允许的更新，CLI 会先说明，并把它加进 profile 的 `pnpm-workspace.yaml` 里的 `minimumReleaseAgeExclude`。技能在同一个包里，随它一起更新。
...
2. 解压。解出来的文件夹名带后缀（比如 `volens-0.4.0`、`volens-main`），**把它改名为 `volens`**
