---
name: volens
description: Set up and maintain a documentation structure that survives a moving design — a contract module inside the project's instruction file, CLAUDE.md on Claude Code and AGENTS.md on Codex, both on DSH (documentation model + working agreements), docs/DECISION-LOG.md (append-only design decision log), and docs/DESIGN.md (derived current-state snapshot kept fresh automatically by a hook). Use when starting work on a new or unfamiliar codebase, when recording a design decision, or when docs need a refresh.
license: MIT
---
...
1. **The project's instruction file** — auto-loaded every session: `CLAUDE.md` on Claude Code, `AGENTS.md` on Codex, and on DSH (`AGENTS.md` by default — DSH loads both `AGENTS.md` and `CLAUDE.md`, and deduplicates them only when their content is identical). volens does not author the file; it maintains one **contract module** inside it (the documentation model + working agreements, wrapped in markers). Repo facts and conventions are the user's to supply (or their agent's init command). The module is English when volens creates the file, and follows the file's dominant language when inserted into an existing one.
...
The layers are split by lifetime: the instruction file holds what is stable (of which volens maintains only its contract module), the log accumulates history without rewriting, and DESIGN.md is a derived snapshot of the latest state.
...
Content under `docs/` (log entries and DESIGN.md) is written in the project's **content language**: `.volens/lang` if present — projects scaffolded before the pin moved keep it at `.claude/volens.lang`, which still counts — else the user-level preference `${XDG_CONFIG_HOME:-$HOME/.config}/volens/lang` (default `en`). The language volens *talks* to the user in is always that user-level preference, independent of any project. The instruction file's language is the project's own; the contract module follows it (English when volens creates the file).
...
2. **Set the two languages.** volens separates *communication* (how it talks to the user) from *content* (what language this project's docs are written in).
   - **Communication language — the user-level preference.** Read `${XDG_CONFIG_HOME:-$HOME/.config}/volens/lang` — `XDG_CONFIG_HOME` wins when it is set and non-empty, as it does for other CLIs. If unset, ask the user in one line (e.g. "文档用什么语言?中文或 English?") and write the answer there — a one-time, per-machine preference. It lives outside every project and outside the plugin directory, so updating volens never touches it. Every word volens says in this scaffold — questions, explanations, the closing report — uses it. No project overrides it: in a project whose docs are English, volens still talks to the user in the user's own language.
   - **Content language — the project `.lang`.** Read `<project>/.volens/lang`; a project scaffolded before the pin moved keeps it at `<project>/.claude/volens.lang`, which still counts. If either exists, that is this project's documentation language — do not ask again. If neither does, ask once, framed in the communication language and offering it as the default ("这个项目的文档用什么语言记录?默认同你的设置"), then write the answer into `.volens/lang` **as a language tag** — `zh`, `en`, `pt-BR` — and not as the language's own name: the hook reads that file as a tag and ignores anything else (a pin it cannot read behaves like a missing one, and the next sync notice says so). That file is a *committed project decision*: it ships in the repo, so every contributor and future clone shares it. A project decides once — the file's presence *is* the "already asked" signal.
   - Neither language governs the instruction file: the contract module follows the rule in step 4.
...
4. **Ensure the contract module is present in the project's instruction file** — the file this harness auto-loads: `CLAUDE.md` on Claude Code, `AGENTS.md` on Codex, and on DSH (`AGENTS.md` by default — DSH loads both `AGENTS.md` and `CLAUDE.md`, and deduplicates them only when their content is identical) — using `references/instruction-file-contract.md`, the canonical English module, wrapped in `<!-- volens contract start/end -->` markers. Do not author any other content of that file.
   - **No such file exists** → create one containing only the module, in English (the canonical block as-is).
   - **The file exists** → insert or refresh only the module: if the markers are present, replace everything between them (inclusive) with the current module; if not, append the module at the end. Write the module in the file's dominant language — translate the canonical block, headings included. If the file's language is genuinely mixed, ask rather than guess.
   - **Both instruction files exist** (a project used by more than one harness) → maintain both, each by the rules above; never copy one over the other.
...
7. **Make the hook effective in this project.** The sync script ships with the plugin — **nothing is copied into the project and no settings file is edited.** One thing remains:
   ...
   The hook then runs on its own — at every `Stop` and `SessionStart` on Claude Code, at every `UserPromptSubmit` on Codex, at the head of every turn on DSH. The three are not the same timing and each is deliberate: Claude Code's two events are genuinely different moments (a turn's reply, a session's start), Codex keeps only `UserPromptSubmit` because its `SessionStart` fires at a turn's head too and would inject the same delta twice, and DSH keeps only the turn head because that is where its injection channel is. State this in the report.
8. **Report.** Summarize what was created, what still needs filling, and open questions. State plainly which instruction file the contract module went into, what it contains, and what language it was written in, and that this project's docs language is pinned in `.volens/lang` (commit that file with the repo). Where the harness has a convention for a gitignored local file beside it (Claude Code: `CLAUDE.local.md`), offer to keep personal notes there rather than in team-shared files. On DSH the skill is invoked as `/volens` — that harness has no plugin namespace.

### Append (a design decision changes)

1. Add a new dated entry to `docs/DECISION-LOG.md` (Context → Decision → Consequences; mark `Supersedes:` when replacing an older decision). Write the entry in the project's content language (`.volens/lang`, else the user-level `${XDG_CONFIG_HOME:-$HOME/.config}/volens/lang`). That is the whole job — DESIGN.md follows on its own.
...
2. Re-sync the contract module in the project's instruction file: if the markers are gone (e.g. a later `/init` rewrite dropped them), re-insert; if stale, replace between the markers with the current module from `references/instruction-file-contract.md`. Match the file's dominant language. If both instruction files carry a block, check both. Do not touch the rest of either file.
3. Move the content-language pin if it is still at the legacy path: write `.volens/lang` with the same value, then remove `.claude/volens.lang`. Housekeeping, not a fix — the hook reads either path.

## Rules

- volens touches an instruction file only through its contract module (markers to markers). The module is English when volens creates the file and follows the file's dominant language when inserted into an existing one; the rest of the file's language and length are not constrained.
...
- Content under `docs/` (log entries and DESIGN.md) is written in the project's content language: `.volens/lang` if present (older projects: `.claude/volens.lang`), else the user-level `${XDG_CONFIG_HOME:-$HOME/.config}/volens/lang` (default `en`).
- The language volens *talks* to the user in is always the user-level preference, never the project's content language.
- `.volens/lang` is a committed project decision — never gitignore it; when it is absent from both paths, ask the project's content language once (never silently default), framed in the user-level language.
- Never put secrets or credentials into an instruction file or docs — they get committed to the repo.
