...
7. **Make the hook effective in this project.** The sync script ships with the plugin — **nothing is copied into the project and no settings file is edited.** One thing remains:
   ...
   The hook then runs on its own, at every `Stop` and `SessionStart`. State this in the report.
...
1. Add a new dated entry to `docs/DECISION-LOG.md` (Context → Decision → Consequences; mark `Supersedes:` when replacing an older decision). Write the entry in the project's content language (`.claude/volens.lang`, else the user-level `~/.config/volens/lang`). That is the whole job — DESIGN.md follows on its own.
