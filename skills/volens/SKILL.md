...
2. **Set the two languages.** volens separates *communication* (how it talks to the user) from *content* (what language this project's docs are written in).
   ...
   - **Content language — the project `.lang`.** Read `<project>/.volens/lang`; a project scaffolded before the pin moved keeps it at `<project>/.claude/volens.lang`, which still counts. If either exists, that is this project's documentation language — do not ask again. If neither does, ask once, framed in the communication language and offering it as the default ("这个项目的文档用什么语言记录?默认同你的设置"), then write the answer into `.volens/lang`. That file is a *committed project decision*: it ships in the repo, so every contributor and future clone shares it. A project decides once — the file's presence *is* the "already asked" signal.
