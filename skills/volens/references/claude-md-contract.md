...
- **CLAUDE.md** — this file, auto-loaded every session. This contract block is the docs-system part of it; the project's stable facts and conventions live here too.
...
- Content under `docs/` (log entries and DESIGN.md updates) is written in this project's content language: `.claude/volens.lang` if present, else the user-level `~/.config/volens/lang` (default `en`).
